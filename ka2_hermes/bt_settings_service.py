...
TUNING_STEPS = {
  ...
  "VIS_TURN_ACC_RESTORE": 1.0,              # 0/1 toggle
  "VIS_TURN_ACC_A_LAT": 0.1,                # m/s^2: 1.2 -> 1.3 -> ... on a 1.2-2.5 range
  "VIS_TURN_ACC_TRIGGER_S": 0.5,            # half a second per press
  "VIS_TURN_ACC_LOOKAHEAD_MAX": 10.0,       # 10 m per press on an 80-320 m range
  "VIS_TURN_ACC_MIN_RADIUS": 25.0,          # 25 m of radius per press
  "VIS_TURN_ACC_MIN_V_KMH": 5.0,            # one ACC step per press
...
def log_refusal(command, reply, raw=""):
  ...
  The RAW line is part of the record: logging only the command word turned "the app's ACC button
  cannot activate ACC" into eight identical `REFUSED ACC -> ERR usage` lines with no way to tell what
  the app had actually sent (24 Sep 2026).
  """
  ...
  log("REFUSED %s -> %s%s" % (command[:60], reply[:80],
                              (" raw=%s" % wifi_redact(raw)[:80]) if raw else ""))
        ...
        while b"\n" in self.buf:
          ...
          for reply in handle(line):
            if reply:
              ...
              if command == "ACC" and (reply.startswith("ERR") or "unknown request" in reply):
                # The app's ACC button is the one control whose request text we cannot see from the
                # phone end, so record it verbatim (redacted) whether it was refused or unrecognised.
                log("acc request %s: %s -> %s" % (self.label, wifi_redact(line)[:80], reply[:80]))
              if reply.startswith("ERR") and command not in ("", "POSE"):
                log_refusal(command, reply, line)
    except Exception as exc:
...
ADAPTER_RECHECK_S = 20
_ADAPTER_LAST = {"Powered": None, "Discoverable": None, "Pairable": None}


def adapter_read(props):
  return {n: bool(props.Get("org.bluez.Adapter1", n)) for n in ("Powered", "Discoverable", "Pairable")}


def adapter_props(bus):
  """Write the adapter's properties, then READ THEM BACK and log what the adapter says.

  Discoverable is written LAST, and that ordering is the fix for a real failure: written before
  DiscoverableTimeout, every Set returned success while `bluetoothctl show` reported Discoverable: no
  minutes later - so the box was invisible to the phone's app and nothing in the log admitted it
  (24 Sep 2026). Reading back is the difference between "we asked" and "it is on".

  `bus` is passed in rather than read from the module: the connection is created inside main().
  """
  props = dbus.Interface(bus.get_object("org.bluez", ADAPTER_PATH), "org.freedesktop.DBus.Properties")
  for name, value in (("Powered", dbus.Boolean(True)), ("Pairable", dbus.Boolean(True)),
                      ("DiscoverableTimeout", dbus.UInt32(0)), ("Discoverable", dbus.Boolean(True))):
    props.Set("org.bluez.Adapter1", name, value)
  got = adapter_read(props)
  _ADAPTER_LAST.update(got)
  log("adapter: powered=%s discoverable=%s pairable=%s timeout=0" % (
      got["Powered"], got["Discoverable"], got["Pairable"]))
  return got


def adapter_keepalive(bus):
  """Put the adapter back if anything else changes it; never raise into the main loop.

  BlueZ drops discoverable on its own and nothing else on this box re-applies it, which leaves the
  phone unable to find the box until someone restarts the service. Only logs when something moved.
  """
  try:
    props = dbus.Interface(bus.get_object("org.bluez", ADAPTER_PATH),
                           "org.freedesktop.DBus.Properties")
    cur = adapter_read(props)
    if cur != _ADAPTER_LAST:
      log("adapter changed to %s - re-applying" % cur)
      adapter_props(bus)
  except Exception as exc:
    log("adapter re-assert failed: %s" % exc)
  return True
...
def main():
  ...
  # Adapter state, re-applied on every start and then kept: a discoverable timeout of 180 s (the
  # default) is why the box kept disappearing from scans, and hciconfig's class does not survive a
  # bluetoothd restart. Setting them here means the unit owns them.
  try:
    adapter_props(bus)
  except Exception as exc:
    log("ADAPTER FAILED: %s" % exc)
  GLib.timeout_add_seconds(ADAPTER_RECHECK_S, adapter_keepalive, bus)

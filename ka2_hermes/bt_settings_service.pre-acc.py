...
_TUNING_CACHE = {"at": 0.0, "limits": {}, "bases": {}, "supported": False}


def controlsd_tuning(max_age_s=10.0):
  """Parse the deployed controlsd.py for its tuning limits, defaults and whether it reads the file.

  Parsed with `ast` (never exec'd) so a service on a car cannot run tuning code by accident.
  """
  now = time.time()
  if now - _TUNING_CACHE["at"] < max_age_s and _TUNING_CACHE["limits"]:
    return _TUNING_CACHE
  limits, bases, supported = {}, {}, False
  try:
    with open(CONTROLD) as fh:
      source = fh.read()
    supported = "TUNING_PATH" in source and "def read_tuning" in source
    tree = ast.parse(source)
    for node in tree.body:
      if isinstance(node, ast.Assign) and len(node.targets) == 1 and isinstance(node.targets[0], ast.Name):
        name = node.targets[0].id
        if name in TUNING_TITLES and isinstance(node.value, ast.Constant):
          bases[name] = float(node.value.value)
          ...
          for key, value in zip(node.value.keys, node.value.values):
            if isinstance(key, ast.Constant) and isinstance(value, ast.Tuple) and len(value.elts) == 2:
              try:
                ...
                continue
  except Exception as exc:                          # a missing or unparseable tree is not fatal
    log("tuning introspection failed: %s" % exc)
  if not limits or not bases or not supported:
    limits, bases, supported = {}, {}, False
...
def tuning_rows():
  ...
  for name in sorted(info["limits"]):
    ...
    entries.append({
      ...
      "mode": "live" if info["supported"] else "inert",
